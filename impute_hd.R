library(hot.deck)
# impute with multiple hot deck and analyze the result
impute_hd <- function(n_count, n_x, ampdf, m=5, 
                      method = "best.cell",
                      sdCutoff=1, alpha=0.05, lm_expr){
  # mean
  ...
  for (i in 1:n_count){
    ...
    imp_list[[i]] <- imp
    for (j in 1:m){
      result[[j]] <- with(imp$data[[j]], eval(parse(text = expr)))
    }
